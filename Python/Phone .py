...
def initial_phonebook():
    rows,cols=int(input("Pleas eneter initial number of contacts:"))
    phone_book=[]
    print("Phone_Book")
    for i in range(rows):
        print("/nEneter contact %d details in the following order(ONLY:)"% (I+1))
        print("Note:* indicate mandatory fields")
        print("........................................................................................")
        temp = []
        for j in range(cols):
            temp.append(str(input("Eneter name*:")))
            if temp[j]=='' or temp[j]=='':
                sys.exit(
                    "Nmae is a mandatory field. process exiting due to blank field..." )
            if j == 1:
            ...
            if j == 2:
                ...
                if temp[j]==''or temp[j]=='':
                    temp[j]=None
            if j == 3:
                temp.append(str(input("Eneter date of birth(DD/MM/YYYY): ")))
                if temp[j]==''or temp[j]=='':
                    temp[j]=None
            if j == 4:
                temp.append(str(input("Eneter catagory(Family/Friends/Work/Others): ")))
                if temp[j]==''or temp[j]=='':
                    temp[j]=None
